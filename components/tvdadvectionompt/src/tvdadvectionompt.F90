...
module tvdadvectionompt_mod
  ...
  integer, dimension(3) :: local_domain_start_index, local_domain_end_index

  public tvdadvectionompt_get_descriptor

  !$omp declare target(flux_x, flux_y, flux_z)
...
contains
  ...
  type(component_descriptor_type) function tvdadvectionompt_get_descriptor()
    tvdadvectionompt_get_descriptor%name="tvd_advection_ompt"
    tvdadvectionompt_get_descriptor%version=0.1
    tvdadvectionompt_get_descriptor%initialisation=>initialisation_callback
    tvdadvectionompt_get_descriptor%timestep=>timestep_callback
    tvdadvectionompt_get_descriptor%finalisation=>finalisation_callback

    tvdadvectionompt_get_descriptor%field_value_retrieval=>field_value_retrieval_callback
    tvdadvectionompt_get_descriptor%field_information_retrieval=>field_information_retrieval_callback
    allocate(tvdadvectionompt_get_descriptor%published_fields(5+11+11+1))
    tvdadvectionompt_get_descriptor%published_fields(1)="u_advection"
    tvdadvectionompt_get_descriptor%published_fields(2)="v_advection"
    tvdadvectionompt_get_descriptor%published_fields(3)="w_advection"
    tvdadvectionompt_get_descriptor%published_fields(4)="th_advection"
    tvdadvectionompt_get_descriptor%published_fields(5)="q_advection"
    

    tvdadvectionompt_get_descriptor%published_fields(5+1)="tend_u_tvdadvection_3d_local"
    tvdadvectionompt_get_descriptor%published_fields(5+2)="tend_v_tvdadvection_3d_local"
    tvdadvectionompt_get_descriptor%published_fields(5+3)="tend_w_tvdadvection_3d_local"
    tvdadvectionompt_get_descriptor%published_fields(5+4)="tend_th_tvdadvection_3d_local"
    tvdadvectionompt_get_descriptor%published_fields(5+5)="tend_qv_tvdadvection_3d_local"
    tvdadvectionompt_get_descriptor%published_fields(5+6)="tend_ql_tvdadvection_3d_local"
    tvdadvectionompt_get_descriptor%published_fields(5+7)="tend_qi_tvdadvection_3d_local"
    tvdadvectionompt_get_descriptor%published_fields(5+8)="tend_qr_tvdadvection_3d_local"
    tvdadvectionompt_get_descriptor%published_fields(5+9)="tend_qs_tvdadvection_3d_local"
    tvdadvectionompt_get_descriptor%published_fields(5+10)="tend_qg_tvdadvection_3d_local"
    tvdadvectionompt_get_descriptor%published_fields(5+11)="tend_tabs_tvdadvection_3d_local"

    tvdadvectionompt_get_descriptor%published_fields(5+11+1)="tend_u_tvdadvection_profile_total_local"
    tvdadvectionompt_get_descriptor%published_fields(5+11+2)="tend_v_tvdadvection_profile_total_local"
    tvdadvectionompt_get_descriptor%published_fields(5+11+3)="tend_w_tvdadvection_profile_total_local"
    tvdadvectionompt_get_descriptor%published_fields(5+11+4)="tend_th_tvdadvection_profile_total_local"
    tvdadvectionompt_get_descriptor%published_fields(5+11+5)="tend_qv_tvdadvection_profile_total_local"
    tvdadvectionompt_get_descriptor%published_fields(5+11+6)="tend_ql_tvdadvection_profile_total_local"
    tvdadvectionompt_get_descriptor%published_fields(5+11+7)="tend_qi_tvdadvection_profile_total_local"
    tvdadvectionompt_get_descriptor%published_fields(5+11+8)="tend_qr_tvdadvection_profile_total_local"
    tvdadvectionompt_get_descriptor%published_fields(5+11+9)="tend_qs_tvdadvection_profile_total_local"
    tvdadvectionompt_get_descriptor%published_fields(5+11+10)="tend_qg_tvdadvection_profile_total_local"
    tvdadvectionompt_get_descriptor%published_fields(5+11+11)="tend_tabs_tvdadvection_profile_total_local"
    
    tvdadvectionompt_get_descriptor%published_fields(5+11+11+1)="tracer_advection"


  end function tvdadvectionompt_get_descriptor
  ...
  subroutine field_value_retrieval_callback(current_state, name, field_value)
    ...
    if (name .eq. "u_advection") then
      ...
      allocate(field_value%real_1d_array(size(th_advection, 1)), source=th_advection(:,current_state%column_local_y,current_state%column_local_x))
    else if (name .eq. "q_advection") then
      allocate(field_value%real_2d_array(size(q_advection, 1), size(q_advection, 4)), source=q_advection(:,current_state%column_local_y,current_state%column_local_x,:))
    else if (name .eq. "tracer_advection") then
  ...
  subroutine initialisation_callback(current_state)
    ...
    allocate(q_advection(size(current_state%zq(1)%data, 1), size(current_state%zq(1)%data, 2), size(current_state%zq(1)%data, 3), size(current_state%zq, 1)))

    local_domain_start_index=current_state%local_grid%local_domain_start_index
  ...
  subroutine timestep_callback(current_state)
    ...
    if (advect_th) call advect_theta(current_state, current_state%u, current_state%v, current_state%w, current_state%zth, &
                          current_state%th, current_state%sth, current_state%global_grid, current_state%local_grid, &
                          current_state%parallel, current_state%halo_column, current_state%field_stepping)
    if (advect_q) call advect_q_fields(current_state, current_state%u, current_state%v, current_state%w, current_state%zq, &
                          current_state%q, current_state%sq, current_state%global_grid, current_state%local_grid, &
                          current_state%parallel, current_state%halo_column, current_state%field_stepping, current_state%number_q_fields)
  ...
  !! @param current_state The current model state_mod
  subroutine advect_q_fields(current_state, u,&
            v, w, zq, q, sq, global_grid,&
            local_grid, parallel, halo_column, field_stepping, number_q_fields)
    type(model_state_type), intent(inout) :: current_state
    type(prognostic_field_type), intent(inout) :: u, w, v
    type(prognostic_field_type), dimension(:), intent(inout) :: zq, q, sq
    ...
    integer, intent(in) :: field_stepping, number_q_fields
    integer :: x, y
  ...
  !! @param current_state The current model state_mod
  subroutine advect_theta(current_state, u,&
            v, w, zth, th, sth, global_grid,&
            local_grid, parallel, halo_column, field_stepping)
    type(model_state_type), intent(inout) :: current_state
    type(prognostic_field_type), intent(inout) :: u, w, v, zth, th, sth
    type(global_grid_type), intent(inout) :: global_grid
    type(local_grid_type), intent(inout) :: local_grid
    type(parallel_state_type), intent(inout) :: parallel
    logical, intent(in) :: halo_column
  ...
  !$OMP target enter data map(to: u,v, w, zth, th, sth, flux_y, flux_z, flux_x)
    ...
    CALL SYSTEM_CLOCK(COUNT=count_end_transfer_to)
      ...
      th_advection=sth%data(:, :, :)
    end if
  end subroutine advect_theta

    subroutine differentiate_face_values_with_wrap_send_recv(y_local_index, x_local_index, u, v, w, q_field, source_field, &
       global_grid, local_grid, parallel, halo_column)

    integer, intent(in) ::y_local_index, x_local_index
    logical, intent(in) :: halo_column
    type(prognostic_field_type), intent(inout) :: u, w, v, q_field, source_field
    type(global_grid_type), intent(inout) :: global_grid
    type(local_grid_type), intent(inout) :: local_grid
    type(parallel_state_type), intent(inout) :: parallel
  ...
  logical function determine_if_advection_here(field)
    ...
    if (len_trim(field) .ne. 0) then
      if (trim(field) .eq. "tvdompt" .or. trim(field) .eq. "any") then
...
end module tvdadvectionompt_mod
